...
const Admin = {
  ...
  async findById(id, { excludePassword = true } = {}) {
    ...
    if (admin && excludePassword) delete admin.password;
    return admin;
  },

  async find() {
    const result = await pool.query(
      'SELECT id, name, email, is_active, created_at, updated_at FROM admins ORDER BY created_at DESC'
    );
    return result.rows;
  },
  ...
  async findByIdAndUpdate(id, { name, email, isActive, password }) {
    ...
    if (fields.length === 0) return await Admin.findById(id);
